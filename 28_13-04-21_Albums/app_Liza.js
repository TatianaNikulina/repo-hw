...
const makeUserFromObject = (user) => {
  const block = document.createElement("div");
  const us = document.createElement("h2");
  us.innerText = user.name;
  block.appendChild(us);
  const nick = document.createElement("h2");
  nick.innerText = user.username;
  block.appendChild(nick);
  const email = document.createElement("p");
  email.innerText = user.email;
  block.appendChild(email);
  block.appendChild(getAlbums(user.id));

  return block;
};

const getAlbums = (usId) => {
  const div = document.createElement("div");
  const filteredAlbums = albums.filter((album) => album.userId === usId);
  if (!filteredAlbums.length) {
    div.innerText = "No Albums";
    return div;
  }
  filteredAlbums.forEach((album) => {
    const block = document.createElement("div");
    const header = document.createElement("h3");
    header.innerText = album.title;
    block.appendChild(header);
    block.appendChild(getPhotos(album.id));
    div.appendChild(block);
  });
  return div;
};

const getPhotos = (albId) => {
  const div = document.createElement("div");
  const filteredPhotos = photos.filter((photo) => photo.albumId === albId);
  if (!filteredPhotos.length) {
    div.innerText = "No Photos";
    return div;
  }
  filteredPhotos.forEach((photo) => {
    const block = document.createElement("div");
    const ph = document.createElement("img");
    ph.src = photo.thumbnailUrl;
...
users.forEach((user) => {
  sect.appendChild(makeUserFromObject(user));
});

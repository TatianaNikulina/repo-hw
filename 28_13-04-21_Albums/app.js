...
const createUserInfo = (user) => {
  ...
  article.id = "user_" + user.id;
  const title = document.createElement("h2");
  title.innerText = user.name;
  article.appendChild(title);
  // const subtitle = document.createElement("h4");
  // subtitle.innerText = `Username: ${user.username}, E-Mail: ${user.email}`;
  // article.appendChild(subtitle);
  leftSection.appendChild(article);

  return article;
};

const getAlbums = (someUserId) => {
  const div = document.createElement("div");
  const filteredAlbums = albums.filter((album) => album.userId === someUserId);
  ...
  filteredAlbums.forEach((album) => {
    ...
    div.appendChild(p);
    div.appendChild(getPhotos(album.id));
  });
  return div;
};

const getPhotos = (someAlbumId) => {
  const div = document.createElement("div");
  const filteredPhotos = photos.filter(
    (photo) => photo.albumId === someAlbumId
  );
  if (!filteredPhotos.length) {
    div.innerText = "Album has no Photos";
    return div;
  }
...
users.forEach((user) => {
  leftSection.appendChild(createUserInfo(user));
});
const albumFromUser_arr = document.querySelectorAll("#left > article");
for (element of albumFromUser_arr) {
  element.addEventListener("click", (event) => {
    const id = +event.currentTarget.id.split("_")[1];
    rightSection.appendChild(getAlbums(id));
  });
}
